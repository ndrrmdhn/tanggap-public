...
const Carousel = React.forwardRef(({ orientation = "horizontal", opts, setApi, plugins, className, children, ...props }, ref) => {
  ...
  const scrollTo = React.useCallback((index) => api && api.scrollTo(index), [api]);
        ...
        <div className="  flex justify-center gap-x-4 pt-5">
          {scrollSnaps.map((_, index) => (
            <DotButton
              ...
              className={cn("w-3 h-3 bg-slate-400 rounded-full", {
                "bg-color-1": index === selectedIndex,
              })}
            />
          ))}
        </div>
